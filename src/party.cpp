...
bool Party::leaveParty(Player* player)
{
	...
	broadcastPartyMessage(MESSAGE_INFO_DESCR, fmt::format("{} has left the party.", player->getName()));
...
bool Party::passPartyLeadership(Player* player)
{
	...
	broadcastPartyMessage(MESSAGE_INFO_DESCR, fmt::format("{} is now the leader of the party.", player->getName()), true);
...
bool Party::joinParty(Player& player)
{
	...
	broadcastPartyMessage(MESSAGE_INFO_DESCR, fmt::format("{} has joined the party.", player.getName()));
	...
	player.sendTextMessage(MESSAGE_INFO_DESCR, fmt::format("You have joined {}'{} party. Open the party channel to communicate with your companions.", leaderName, leaderName.back() == 's' ? "" : "s"));
	return true;
}
...
{
	player.sendTextMessage(MESSAGE_INFO_DESCR, fmt::format("{} has revoked {} invitation.", leader->getName(), leader->getSex() == PLAYERSEX_FEMALE ? "her" : "his"));
	leader->sendTextMessage(MESSAGE_INFO_DESCR, fmt::format("Invitation for {} has been revoked.", player.getName()));
...
bool Party::invitePlayer(Player& player)
{
	...
	if (empty()) {
		leader->sendTextMessage(MESSAGE_INFO_DESCR, fmt::format("{} has been invited. Open the party channel to communicate with your members.", player.getName()));
		g_game.updatePlayerShield(leader);
		leader->sendCreatureSkull(leader);
	} else {
		leader->sendTextMessage(MESSAGE_INFO_DESCR, fmt::format("{} has been invited.", player.getName()));
	}
	...
	player.sendTextMessage(MESSAGE_INFO_DESCR, fmt::format("{} has invited you to {} party.", leader->getName(), leader->getSex() == PLAYERSEX_FEMALE ? "her" : "his"));
	return true;
}
