...
bool IOBan::isAccountBanned(uint32_t accountId, BanInfo& banInfo)
{
	...
	DBResult_ptr result = db.storeQuery(fmt::format(
		"SELECT `reason`, `expires_at`, `banned_at`, `banned_by`, (SELECT `name` FROM `players` WHERE `id` = `banned_by`) AS `name` FROM `account_bans` WHERE `account_id` = {}", accountId));
	if (!result) {
		return false;
	}
	...
	if (expiresAt != 0 && time(nullptr) > expiresAt) {
		...
		g_databaseTasks.addTask(fmt::format(
			...
			accountId, db.escapeString(result->getString("reason")), result->getNumber<time_t>("banned_at"), expiresAt, result->getNumber<uint32_t>("banned_by")));
...
bool IOBan::isIpBanned(uint32_t clientIP, BanInfo& banInfo)
{
	...
	DBResult_ptr result = db.storeQuery(fmt::format(
		"SELECT `reason`, `expires_at`, (SELECT `name` FROM `players` WHERE `id` = `banned_by`) AS `name` FROM `ip_bans` WHERE `ip` = {}", clientIP));
	if (!result) {
		return false;
	}
	...
	if (expiresAt != 0 && time(nullptr) > expiresAt) {
		g_databaseTasks.addTask(fmt::format(
			"DELETE FROM `ip_bans` WHERE `ip` = {}", clientIP));
		return false;
	}
...
bool IOBan::isPlayerNamelocked(uint32_t playerId)
{
	return Database::getInstance().storeQuery(fmt::format(
		"SELECT 1 FROM `player_namelocks` WHERE `player_id` = {}", playerId)).get() != nullptr;
}
