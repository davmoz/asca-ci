...
bool DatabaseManager::optimizeTables()
{
	...
	do {
		...
		if (db.executeQuery(fmt::format("OPTIMIZE TABLE `{}`", tableName))) {
...
bool DatabaseManager::tableExists(const std::string& tableName)
{
	...
	return db.storeQuery(fmt::format(
		"SELECT `TABLE_NAME` FROM `information_schema`.`tables` WHERE `TABLE_SCHEMA` = {} AND `TABLE_NAME` = {} LIMIT 1",
		db.escapeString(g_config.getString(ConfigManager::MYSQL_DB)), db.escapeString(tableName))).get() != nullptr;
}
...
bool DatabaseManager::isDatabaseSetup()
{
	...
	return db.storeQuery(fmt::format(
		...
		db.escapeString(g_config.getString(ConfigManager::MYSQL_DB)))).get() != nullptr;
}
...
void DatabaseManager::updateDatabase()
{
	...
	do {
		auto migrationFile = fmt::format("data/migrations/{}.lua", version);
...
bool DatabaseManager::getDatabaseConfig(const std::string& config, int32_t& value)
{
	...
	DBResult_ptr result = db.storeQuery(fmt::format(
		"SELECT `value` FROM `server_config` WHERE `config` = {}", db.escapeString(config)));
	if (!result) {
		return false;
	}
