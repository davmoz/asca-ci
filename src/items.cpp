...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"spamable", 	ITEM_PARSE_SPAMABLE},
	{"attackmodpassive", ITEM_PARSE_ATTACKMODPASSIVE },
	{"healmodpassive", ITEM_PARSE_HEALMODPASSIVE },
	{"armpenpercents", ITEM_PARSE_ARMPENPERCENTS },
	{"critpassive", ITEM_PARSE_CRITPASSIVE },
	{"defensemodpassive", ITEM_PARSE_DEFENSEMODPASSIVE },
	{"attackspeedbonus", ITEM_PARSE_ATTACKSPEEDBONUS },
	{"maxhealthpercent", ITEM_PARSE_MAXHEALTHPERCENT },
	{"maxmanapercent", ITEM_PARSE_MAXMANAPERCENT },
	{"respenpercents", ITEM_PARSE_RESPENPERCENTS },
	{"enchantedarrow", ITEM_PARSE_ENCHANTEDARROW },
	{"enchantedbolt", ITEM_PARSE_ENCHANTEDBOLT },
	{"bullet", ITEM_PARSE_BULLET },
	{"firespark", ITEM_PARSE_FIRESPARK },
	{"maxmanapercent", ITEM_PARSE_MAXMANAPERCENT },
	{"poisonstrike", ITEM_PARSE_POISONSTRIKE },
	{"iceclaw", ITEM_PARSE_ICECLAW },
	{"grenade", ITEM_PARSE_GRENADE },
	{"resist", ITEM_PARSE_RESIST },
	{"blockcurses", ITEM_PARSE_BLOCKCURSES },
	{"dodgepassive", ITEM_PARSE_DODGEPASSIVE },
	{"repellingpassive", ITEM_PARSE_REPELLINGPASSIVE },
	{"reflectpercentmagic", ITEM_PARSE_REFLECTPERCENTMAGIC },
	{"reflectpercentphysical", ITEM_PARSE_REFLECTPERCENTPHYSICAL },
	{"healModPassive", ITEM_PARSE_HEALMODPASSIVE },
	{"magdef", ITEM_PARSE_MAGDEF },
	{"reflectchancemagic", ITEM_PARSE_REFLECTCHANCEMAGIC },
	{"reflectchancephysical", ITEM_PARSE_REFLECTCHANCEPHYSICAL },
	{"partofset", ITEM_PARSE_PARTOFSET },
		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			case ITEM_PARSE_EXTRADEF: {
				...
				break;
									...
									// Nya cases från ascalon
			case ITEM_PARSE_GRADE: {
				it.grade = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_SPAMABLE: {
				it.spamable = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_ATTACKMODPASSIVE: {
				it.attackModPassive = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_HEALMODPASSIVE: {
				it.healModPassive = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_ARMPENPERCENTS: {
				it.armPenPercents = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_CRITPASSIVE: {
				it.critPassive = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_DEFENSEMODPASSIVE: {
				it.defenseModPassive = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_ATTACKSPEEDBONUS: {
				it.attackSpeedBonus = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_MAXHEALTHPERCENT: {
				it.maxhealthpercent = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_MAXMANAPERCENT: {
				it.maxmanapercent = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_RESPENPERCENTS: {
				it.resPenPercents = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_ENCHANTEDARROW: {
				it.enchantedarrow = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_ENCHANTEDBOLT: {
				it.enchantedbolt = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_BULLET: {
				it.bullet = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_FIRESPARK: {
				it.firespark = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_POISONSTRIKE: {
				it.poisonstrike = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_ICECLAW: {
				it.iceclaw = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_GRENADE: {
				it.grenade = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_RESIST: {
				it.resist = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_BLOCKCURSES: {
				it.blockCurses = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_DODGEPASSIVE: {
				it.dodgePassive = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_REPELLINGPASSIVE: {
				it.repellingPassive = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_REFLECTPERCENTMAGIC: {
				it.reflectpercentmagic = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_REFLECTPERCENTPHYSICAL: {
				it.reflectpercentphysical = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}
			...
			case ITEM_PARSE_MAGDEF: {
				it.magdef = pugi::cast<int32_t>(valueAttribute.value());
				break;
			}

			case ITEM_PARSE_REFLECTCHANCEMAGIC: {
				it.reflectchancemagic = pugi::cast<int32_t>(valueAttribute.value());
				break;
			}
			...
			case ITEM_PARSE_PARTOFSET: {
				it.partofset = pugi::cast<uint32_t>(valueAttribute.value());
				break;
			}
			...
			// std::cout << keyAttribute.as_string() << "---" << valueAttribute.as_string() << std::endl;
