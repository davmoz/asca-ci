	...
	for (auto& tileNode : tileAreaNode.children) {
		...
		while (propStream.read<uint8_t>(attribute)) {
			switch (attribute) {
				...
				case OTBM_ATTR_ITEM: {
					...
					if (!item) {
						setLastErrorString(fmt::format("[x:{}, y:{}, z:{}] Failed to create item.", x, y, z));
						return false;
					}
				...
				default:
					setLastErrorString(fmt::format("[x:{}, y:{}, z:{}] Unknown tile attribute.", x, y, z));
					return false;
			}
		}
		...
		for (auto& itemNode : tileNode.children) {
			if (itemNode.type != OTBM_ITEM) {
				setLastErrorString(fmt::format("[x:{}, y:{}, z:{}] Unknown node type.", x, y, z));
				return false;
			}
			...
			if (!item->unserializeItemNode(loader, itemNode, stream)) {
				setLastErrorString(fmt::format("[x:{}, y:{}, z:{}] Failed to load item {}.", x, y, z, item->getID()));
				delete item;
				return false;
			}
